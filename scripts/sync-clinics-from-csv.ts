...
async function syncClinics() {
  console.log("🏥 Syncing clinics from CSV...");
  ...
  console.log(`📊 Clearing old clinics and importing ${records.length} records...`);
  ...
  console.log(`\n✅ Clinics synced: ${inserted} inserted, ${skipped} skipped`);
}

syncClinics().catch(console.error).then(() => process.exit(0));
