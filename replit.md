...
- **Emergency Symptom Knowledge Blocks**: 10 detailed guides covering critical pet conditions (cat panting, dog bloat, poisoning, seizures, limping, not eating, eye injury, heatstroke, urinary blockage, breathing difficulty) with severity classifications and action steps for panic search traffic. **Phase 3 GEO Enhancements**: MedicalWebPage + MedicalCondition schemas, medical disclaimers above-fold, AI-summary blocks (sr-only, bilingual), question-format headings, "What info helps hospitals" definition lists per symptom, lastReviewed footers.
- **Emergency Checklist** (`/emergency-checklist`): GEO-optimized HTML-first checklist for pet owners heading to the vet. Includes: Quick Actions Before Leaving, What to Bring (7 items), Information to Prepare (6 items), During Transport Tips, At the Hospital guidance, bilingual FAQ section. Schema.org MedicalWebPage + FAQPage + BreadcrumbList. Medical disclaimer and AI-summary blocks included.
...
- **Phase 4 Hyper-Local District Emergency Guides** (`/district/:code`): Transformed district pages from simple hospital listings into comprehensive 6-block emergency guides. **Block 1**: Emergency Overview with question-format heading ("What should I do if my pet has an emergency in [District]?"), amber medical disclaimer above-fold, sr-only AI summary. **Block 2**: "What to Know" section with semantic `<dl>` for local transport (MTR lines, tunnel access, taxi availability). **Block 3**: Typhoon & Weather Reality with T8/T10 guidance and link to /blog/typhoon-guide. **Block 4**: Verified 24-Hour Hospitals directory with dynamic count, partner badges, distance display. **Block 5**: PetSOS Bridge with muted CTA and dynamic hospital count. **Block 6**: Trust Footer with lastReviewed date and links to /emergency-symptoms, /emergency-checklist. **Schema**: MedicalWebPage with areaServed (district), medicalAudience (Pet Owner), lastReviewed. **Wave 1-2 Districts**: Eastern, Central & Western, Wan Chai, Kowloon City, Yau Tsim Mong, Sha Tin (with detailed transport data).
