...
- **UX Enhancements**: Prominent pet management CTA, auto-scroll to status button post-broadcast, extended toast duration, blue-styled "View Broadcast Status" button, post-broadcast guidance to status page, compressed VoiceRecorder UI with collapsible tips (info icon), emergency request edit functionality with real-time broadcast message updates, severity-ordered emergency symptoms (critical → serious → moderate), compact clinic action buttons (Call/WhatsApp/Maps) in single horizontal row, and brand-aligned clinic directory with proper bilingual region names, Maps navigation button, red-themed compact cards (~25% more compact) showing more clinics per screen, clickable clinic cards that open Google Maps with business reviews/location in new tab with external link icon and hover effects while preserving independent button functionality, mobile-optimized horizontally scrollable region tabs with hidden scrollbar and larger touch targets (80px min-width, 40px height), enhanced location banner visibility with larger text (text-sm) for better mobile readability, and consistent compact clinic card design across both clinics directory and emergency results pages with red left borders, truncated addresses, and partner clinic prioritization.
- **Partner Clinic Prioritization**: Partner clinics (isSupportHospital=true) are prioritized and displayed first on both the public clinics directory and emergency results pages, with purple/blue gradient badges (⭐ PetSOS Partner) for easy identification. On emergency results, the sorting hierarchy is: existing patient clinics → partner clinics → distance-sorted remaining clinics.
- **Region Management**: Active regions limited to Hong Kong Island (HKI), Kowloon (KLN), and New Territories (NTI). Removed deprecated "Hong Kong" and "Test Region" entries to maintain clean data and prevent confusion in region filters.
